...
**Domain:** Travel-agency marketing site (tour package browsing, no checkout) + internal admin/CRM panel with email/SMS messaging automation
**Researched:** 2026-07-18
**Confidence:** MEDIUM-HIGH (core framework/DB versions verified against npm registry and official docs — HIGH; pricing/free-tier figures verified against official pricing pages plus multiple 2026 aggregator sources — MEDIUM; PH SMS pricing cross-checked across 2 providers — MEDIUM)
...
```bash
# Core
npx create-next-app@latest travelsentro --typescript --tailwind --app --turbopack
cd travelsentro
npm install @supabase/supabase-js @supabase/ssr

# Admin panel UI
npx shadcn@latest init
npx shadcn@latest add button card table dialog form sidebar dropdown-menu badge
...
The project needs (a) transactional email (auto-reply, internal alert) and (b) admin-triggered individual/bulk email to customers. Free-tier limits below are per official pricing pages, confirmed by 2026 aggregator sources:
...
**Recommendation:** Start with **Resend** for its Next.js/React Email integration quality. If the admin's bulk-send behavior (e.g., "email all 400 leads about a promo") regularly exceeds 100/day, either (a) throttle bulk sends client-side to spread across multiple days, or (b) migrate/dual-run with Brevo for bulk marketing sends while keeping Resend for transactional (auto-reply/alert). Flag this as a roadmap decision point once real usage patterns are known — do not over-engineer a multi-provider abstraction on day one.

## PH-Friendly SMS Provider Comparison

No SMS provider has a real free tier — this is the one pay-as-you-go exception per project constraints. For <500 contacts with occasional bulk sends, cost is driven entirely by per-message rate, not any monthly minimum:
...
**Recommendation:** **Semaphore** as primary SMS provider. It is PH-market-specific, has no setup fee, and its 1,000-recipient-per-request API comfortably covers "<500 contacts, occasional bulk sends." Revisit Twilio only if the business expands beyond PH SMS or needs WhatsApp Business API integration later (out of scope for v1 — WhatsApp CTA in this project is just a `wa.me` deep link, not the Business API).

## Formspree Webhook Integration — Important Cost Finding

The project constraint says "new backend needs to receive submissions via webhook," but this needs a flag: **Formspree's Webhook plugin requires a paid plan** (Personal tier, ~$10/month or higher) — it is not available on Formspree's free plan.

Two paths, both valid, to surface as a roadmap/phase decision:

1. **Pay for Formspree's paid tier** (~$10/mo) and use their native Webhook plugin (`formspree.json` `actions: [{ type: "webhook", url: "..." }]` or dashboard UI) to POST JSON to a Next.js Route Handler on every submission. Simplest, most reliable (Formspree owns retry/delivery), but breaks the "free tier wherever possible" constraint for this one piece.
2. **Free workaround — dual-submit from the client.** If the existing Formspree form is wired up via JS/`fetch` (Formspree's AJAX pattern, common with React/Next forms) rather than a raw HTML `<form action="https://formspree.io/...">` POST, add a second `fetch` call from the same submit handler to your own `/api/inquiries` Route Handler, sent in parallel with (or immediately after) the Formspree submission. This achieves the same "submission lands in the CRM" outcome without paying Formspree anything — the tradeoff is it depends on the client's JS running successfully (no server-side retry safety net Formspree would otherwise provide) and requires inspecting/possibly modifying the existing form's submit code.

**This needs to be resolved during the Formspree-integration phase** by first inspecting how the existing form is actually wired (raw HTML POST vs. JS fetch) — that determines whether path 1 or path 2 is viable. Either way, whichever Route Handler receives the data must validate the payload shape with `zod` before writing to Supabase (webhooks provider-agnostic; don't trust the shape).

## Auth & Roles: Supabase Auth vs Custom

**Recommendation: Supabase Auth**, not a custom auth system.
...
- **Roles/permissions model:** For this project's simple 2-role (Admin/Staff) + 3 fixed permission toggles (message customers / manage packages / edit CRM) requirement, skip the more complex "Custom Access Token Auth Hook + JWT app_metadata claims" pattern that's recommended for larger multi-tenant SaaS RBAC. That pattern adds real complexity (Auth Hooks, token refresh subscriptions) that isn't justified by 2 roles and 3 booleans.
  - Instead: a `profiles` table (one row per admin/staff user, keyed to `auth.users.id`) with `role` (`admin`/`staff`) and three boolean columns (`can_message_customers`, `can_manage_packages`, `can_edit_crm`), enforced via RLS policies that check the requesting user's row in `profiles`.
  - Enforce both in RLS (defense in depth at the DB layer) and in the Next.js app layer (hide/disable UI for actions a staff member can't perform) — RLS is not optional even though the admin panel is server-rendered, because Supabase's client libraries can still be called directly with a user's session token.
  - Never use the Supabase **service role key** in code paths reachable from user input (e.g., inside the webhook handler is fine since it's server-only and not user-triggered, but never expose it to any client-executable code) — it bypasses RLS entirely.
...
**If Resend's 100/day free cap becomes a real constraint (bulk sends to a growing lead list):**
...
**If Semaphore reliability or pricing disappoints during the messaging phase:**
...
**If Supabase's free-tier project pause-after-7-days-inactivity becomes disruptive:**
...
---
*Stack research for: Travel-agency marketing site + admin/CRM panel (TravelSentro)*
*Researched: 2026-07-18*
